      ...
      <table className="w-full text-left min-w-[1000px]">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/50">
            {["Candidate", "Rol", "Experience", "Tech Stack", "English", "Location", "Updated", "Links", ""].map((h) => (
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {talents.length === 0 ? (
            ...
            talents.map((t, i) => {
              // El backend puede devolver el rol como "role" (desde la vista) o "Rol" (desde el tipo)
              const rolLabel = (t as any).role ?? t.Rol ?? null;
              ...
              return (
                <tr key={t.candidate_code ?? i} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    {rolLabel
                      ? <span className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded text-xs font-medium border border-slate-200 dark:border-slate-700">{rolLabel}</span>
