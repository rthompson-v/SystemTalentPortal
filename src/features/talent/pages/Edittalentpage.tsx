...
export function EditTalentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: UpdateTalent = {
      ...
      Skillset:     fd.get("Skillset")     as string || undefined,
      EnglishLevel: fd.get("EnglishLevel") ? Number(fd.get("EnglishLevel")) : undefined,
      Experiencia:  fd.get("Experiencia")  ? Number(fd.get("Experiencia"))  : undefined,
      Rol:          fd.get("Rol")          as string || undefined,
      Esquema:      fd.get("Esquema")      as string || undefined,
  ...
  const formattedDate = talent.last_update
    ? new Date(talent.last_update).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
    : null;
      ...
      <div className="max-w-[1120px] mx-auto px-6 py-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Section icon="work" title="Role & Experience">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                ...
                <input name="Rol" defaultValue="" placeholder="e.g. Developer" className={inputCls} type="text" />
              </div>
              <div>
              ...
              <div className="md:col-span-2 lg:col-span-3">
                ...
                <textarea name="Skillset" defaultValue={talent.skillset ?? ""} className={`${inputCls} h-24 resize-none`} />
              </div>
            </div>
          </Section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="Expectativas" defaultValue={talent.costo_expectativa ?? ""} className={`${inputCls} pl-7`} type="text" />
                </div>
              </div>
              <div>
          ...
          <div className="flex items-center justify-between pt-4 pb-12 border-t border-slate-200 dark:border-slate-800">
            {formattedDate && (
              <p className="text-slate-500 text-sm italic">Last updated: {formattedDate}</p>
            )}
            <div className="flex gap-4 ml-auto">
              ...
              <button type="submit" disabled={update.isPending}
                className="px-10 py-3 rounded-lg font-bold text-white bg-[#135bec] hover:bg-[#135bec]/90 shadow-lg transition-all flex items-center gap-2 disabled:opacity-70">
