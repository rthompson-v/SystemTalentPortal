...
export function TalentListPage() {
  ...
  const [results, setResults] = useState<Talent[] | null>(null);
  const lastQueryRef = useRef<SearchFilters | null>(null);

  // Filtro de inglés en cliente
  ...
  const handleSearch = async (filters: SearchFilters) => {
    ...
    // p_q = búsqueda libre (nombre, email, etc.)
    // p_tech = filtro de tecnología separado — va directo a la RPC
    const res = await search.mutateAsync({
      q:     filters.q.trim(),
      limit: 100,
      tech:  filters.technologies.trim(), // campo nuevo
    });
        ...
        {!hasSearched && !search.isPending && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-16 h-16 rounded-2xl bg-[#135bec]/10 flex items-center justify-center mb-4">
              <span className="material-symbols-outlined text-[#135bec]" style={{ fontSize: 32 }}>manage_search</span>
            </div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">
              Usa los filtros para buscar candidatos
            </h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm max-w-md">
              Puedes buscar por nombre, email, skill, tecnología o rango de puntaje de inglés.
            </p>
          </div>
        )}
        ...
        {hasSearched && !search.isPending && (
          <>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-xl font-black text-slate-900 dark:text-white">Resultados</h2>
            ...
            {results!.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
                <span className="material-symbols-outlined text-slate-300 dark:text-slate-600 mb-3" style={{ fontSize: 40 }}>search_off</span>
                <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Sin resultados</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm">Intenta con otros términos de búsqueda.</p>
              </div>
            ) : (
